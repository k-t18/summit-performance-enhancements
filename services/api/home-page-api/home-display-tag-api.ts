...
const getDisplaytagsDataFromAPI = async (token?: any) => {
  const version = CONSTANTS.VERSION;
  const method = "get_tagged_products";
  const entity = "product";
  const params = `?version=${version}&method=${method}&entity=${entity}`;
  const displayTagsList = await callGetAPI(
    `${CONSTANTS.API_BASE_URL}/api/resource/Tag`,
    token
  );
      ...
      displayTagsList?.data?.data?.length > 0 &&
        displayTagsList?.data?.data.map(async (tag: any) => {
          const res = await callGetAPI(
            `${CONSTANTS.API_BASE_URL}/${CONSTANTS.API_MANDATE_PARAMS}${params}&tag=${tag.name}&currency=INR`,
            token
          );
